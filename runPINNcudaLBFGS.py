...
class PINN(nn.Module):
    def __init__(self, x, t, u_train, x_f, t_f, layer_size, neur_size, in_size, out_size, params_list = None):
        ...
        #self.optimizer = optim.SGD(params = self.layers, lr = 0.001, weight_decay = 0.0)
...
model = PINN(X_u_train2[:,0], 
             ...
             2, 
             1).cuda()
optimizer = optim.LBFGS(params = model.parameters(), lr = 1, max_iter = 1000, tolerance_grad = 1e-07, tolerance_change = 1e-09, history_size = 10, line_search_fn=None)#max_eval = None)
...
np.savetxt(r'.\U_final2LBFGS.dat', Results)
#plt.show()
...
plt.savefig('U_final2LBFGS.png', dpi = 150)

#torch.cuda.empty_cache()
#plt.ioff()
# plt.scatter(X_u_train2[:,1].T, X_u_train2[:,0].T, c = u_train2, cmap='coolwarm')
plt.show()
...
# Mat = np.mat(Map)
# #[Tt, Xx, U_final2]
# plt.imshow(U_final3, cmap = 'coolwarm', interpolation = 'nearest')
# plt.colorbar()
# plt.show()
# #scatter = plt.scatter(X2[1,:].detach().numpy(),X2[0,:].detach().numpy(), c = U_final2,cmap='coolwarm')
